...
Default attendance examples are seeded for Roll `101` as:
- `14-Feb`, `09:50`, `PRESENT`
- `14-Feb`, `10:40`, `ABSENT`
...
- **Permission denied: `attendance.xlsx`**
  - Close `attendance.xlsx` in Excel/LibreOffice and retry (file lock prevents writes).
- **`535 Username and Password not accepted`**
  - Enable 2-Step Verification on `n4manphogat@gmail.com` and create a Gmail App Password.
  - Set `GMAIL_APP_PASSWORD` to that 16-character app password (spaces are okay; app removes them).
...
- UI uses a CSS gradient background (no binary image asset), so GitHub/Codex diffs stay text-only.
